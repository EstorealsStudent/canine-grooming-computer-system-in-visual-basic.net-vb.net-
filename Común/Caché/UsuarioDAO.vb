...
and fallecido=0"
    ...
    Public Sub InsertarCita(idMascota, idhorario, idecita, fechacita, descripcion)
        Using connection = GetConnection()
            connection.Open()
            Using command = New SqlCommand()
                command.Connection = connection
                command.CommandText = "INSERT INTO Cita (IDMascota, IDHorario, IDECita, Fechacita, Descripcion, FCreacion)
VALUES (@IDMascota, @IDHorario, @IDECita, @Fechacita, @Descripcion, getdate())"
                ' Asegúrate de configurar los parámetros correspondientes
                command.Parameters.AddWithValue("@IDMascota", idMascota)
...
where c.IDCita=@idcita"
    ...
    Public Sub EditarEstadoCita(idcita, idecita)
...
where IDCita=@idcita"
    ...
    Public Sub EditarCita(idcita, idhorario, fechacita, descripcion)
...
WHERE IDCita = @idcita"
    ...
    Public Sub InsertarRazaMascota(IdTipoMascota, Nombre)

        Using connection = GetConnection()
            connection.Open()
            Using command = New SqlCommand()
                command.Connection = connection
                command.CommandText = "insert into RazaMascota(IDTMascota,Nombre,FCreacion)
values (@idtmascota,@mascota,GETDATE())"
                ' Asegúrate de configurar los parámetros correspondientes
...
order by TipoMascota.Nombre,RazaMascota.Nombre"
    ...
    Public Sub EditarRaza(IdRmazcota, IdTMascota, nombre)
...
VALUES (@PrimerNombre, @SegundoNombre, @ApellidoPaterno, @ApellidoMaterno, @Email, @Usuario, @Contraseña, @IDRol, GETDATE())"
                ' Asegúrate de configurar los parámetros correspondientes
                command.Parameters.AddWithValue("@PrimerNombre", PrimerNombre)
                command.Parameters.AddWithValue("@SegundoNombre", SegundoNombre)
                command.Parameters.AddWithValue("@ApellidoPaterno", ApellidoPaterno)
