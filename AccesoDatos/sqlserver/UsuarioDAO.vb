...
Public Class UsuarioDAO
    ...
    Public Function RequestUserPassword(ByVal requestingUser As String) As String
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    ...
                    command.Connection = connection
                    command.CommandText = "SELECT IDTrabajador, PrimerNombre, SegundoNombre, ApellidoPaterno, ApellidoMaterno, Email, Usuario,CONVERT(NVARCHAR(MAX), DecryptByPassPhrase('petpalace', Clave)) AS Contraseña, IDRol FROM Trabajador where (Usuario=@Usuario or Email=@Email) and Estatus=1"
                    ...
                    "Por favor, revisa tu Correo: " & vbLf & userMail & vbLf


                    Else
                        Return "Lo siento, no existe una cuenta con este" & vbLf & "Nombre de usuario o Correo Electrónico."
                    End If
        ...
        Catch ex As Exception
            MsgBox("Error en la conexión: " & ex.Message)
            Return "Ha ocurrido un error al procesar tu solicitud. Por favor, intenta nuevamente más tarde."
        End Try
        ' Añadir un retorno predeterminado en caso de que ninguna de las condiciones del 'If' se cumpla.
        Return "Ha ocurrido un error al procesar tu solicitud. Por favor, intenta nuevamente más tarde."
    End Function
    ...
    Public Function Login(user As String, pass As String) As Boolean
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT IDTrabajador, PrimerNombre, SegundoNombre, ApellidoPaterno, ApellidoMaterno, Email, Usuario, CONVERT(NVARCHAR(MAX), DecryptByPassPhrase('petpalace', Clave)) AS Contraseña, IDRol FROM Trabajador WHERE (Usuario= @Usuario and CONVERT(varchar(MAX), DECRYPTBYPASSPHRASE('petpalace', Clave))= @Contraseña) and Estatus=1"
    ...
    Public Function Buscarclientes() As List(Of Clientes)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT IDCliente, CONCAT(PrimerNombre, ' ', ISNULL(SegundoNombre, ''), ' ', ApellidoPaterno, ' ', ISNULL(ApellidoMaterno, '')) AS Nombre, Celular FROM Cliente"
                    Dim reader = command.ExecuteReader()
                    If reader.HasRows Then
                        While reader.Read()
    ...
    Public Function MostrarClientes(idclientes) As Clientes
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT c.IDCliente, c.IDGenero, c.PrimerNombre, c.SegundoNombre, c.ApellidoPaterno, c.ApellidoMaterno, c.RFC, c.CURP, c.Celular, c.TelCasa, c.TelTrabajo, c.TelExterno, c.Email, c.Calle, c.NumeroExt, c.Colonia, c.Municipio, c.CodigoPostal, c.Estado, c.Descripcion, g.NombreGenero FROM Cliente c JOIN Genero g ON c.IDGenero = g.IDGenero where c.IDCliente = @idcliente"
    ...
    Public Sub EditarClientes(id, IDGenero, PrimerNombre, SegundoNombre, ApellidoPaterno, ApellidoMaterno, RFC, CURP, Celular, TelCasa, TelTrabajo, TelExterno, Email, Calle, NumeroExt, Colonia, Municipio, CodigoPostal, Estado, Descripcion, IdUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE Cliente SET IDGenero = @genero, PrimerNombre = @primerNombre,SegundoNombre = @segundoNombre,ApellidoPaterno = @ApellidoPaterno,ApellidoMaterno = @ApellidoMaterno, RFC = @RFC, CURP = @Curp, Celular = @celular, TelCasa = @telCasa, TelTrabajo = @telTrabajo, TelExterno = @telexterno, Email = @Email, Calle = @Calle, NumeroExt = @NumeroExt, Colonia = @colonia, Municipio = @municipio, CodigoPostal = @Codigopostal, Estado = @estado, IdUsuarioModifica=@IDUsuarioModifica, FechaModifica=GETDATE(), Descripcion = @Descripcion WHERE IDCliente = @IdCliente"
                    ' Asegúrate de configurar los parámetros correspondientes
    ...
    Public Function CargarMascotasClientes(id) As DataTable
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT M.IDMascota, M.Nombre AS NombreMascota, T.Nombre AS TipoMascota, RM.Nombre AS Raza, M.Peso, SM.Nombre AS Sexo, M.Color, M.Fallecido FROM Mascota M JOIN SexoMascota SM ON M.IDSeMascota=SM.IDSeMascota JOIN RazaMascota RM ON M.IDRMascota = RM.IDRMascota JOIN TipoMascota T ON RM.IDTMascota = T.IDTMascota WHERE IDCliente = @idcliente"
                    command.Parameters.AddWithValue("@idcliente", id)
                    command.CommandType = CommandType.Text
    ...
    Public Function MostrarMascotas(idmascotas) As Mascotas
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT M.IDMascota, M.IDCliente, M.IDRMascota, M.IDSeMascota, M.Nombre AS NombreMascota, M.Peso, M.Color, M.Esvacunado, M.Vacuna, m.VigVacuna, m.foto, m.fallecido, T.IDTMascota FROM Mascota M JOIN RazaMascota RM ON M.IDRMascota = RM.IDRMascota JOIN TipoMascota T ON RM.IDTMascota = T.IDTMascota WHERE IDMascota = @idmascota"
    ...
    Public Function Obtenerrazamascotas(razamascota) As List(Of Mascotas)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select * from RazaMascota where IDRMascota=@idRmascota and Estatus=1 order by Nombre asc"
    ...
    Public Function ObtenerrazaEDITARmascotas(razamascota) As List(Of Mascotas)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select * from RazaMascota where IDTMascota=@idTmascota and Estatus=1 order by Nombre asc"
    ...
    Public Sub Insertarmascotas(idcliente, idrazamascota, sexomascota, nombre, peso, color, esvacunado, vacunainicio, vigvacuna, foto, IDUsuariocrea)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO Mascota (IDCliente, IDRMascota, IDSeMascota, Nombre, Peso, Color, Esvacunado, Vacuna, VigVacuna, Foto, IdUsuarioCrea) VALUES (@IDCliente, @IDRMascota, @IDSeMascota, @Nombre, @Peso, @Color, @Esvacunado, @Vacuna, @VigVacuna, @Foto, @idusuariocrea)"
    ...
    Public Sub Editarrmascotas(idmascota, idcliente, idrazamascota, sexomascota, nombre, peso, color, esvacunado, vacunainicio, vigvacuna, foto, falleció, IDUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE Mascota SET IDCliente = @IDCliente, IDRMascota = @IDRMascota, IDSeMascota = @IDSeMascota, Nombre = @Nombre, Peso = @Peso, Color = @Color, Esvacunado = @Esvacunado, Vacuna = @Vacuna, VigVacuna = @VigVacuna, Foto = @Foto, Fallecido = @Fallecido, IdUsuarioModifica=@idusuariomodifica, FechaModifica=GETDATE() WHERE IDMascota = @IDMascota"
    ...
    Public Sub InsertarServicio(TipoServicio, Nombre, Costo, idusuariocrea)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO Servicio (TipoServicio, Nombre, Costo, IdUsuarioCrea) VALUES (@TipoServicioID, @NombreServicio, @CostoServicio, @idusuariocrea)"
    ...
    Public Function Buscarservicios() As List(Of Servicios)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select s.IDServicio, t.Nombre as 'tipo de servicio', s.Nombre, s.Costo from Servicio s join tipoServicio T on t.TipoServicio=s.TipoServicio order by t.Nombre"
                    Dim reader = command.ExecuteReader()
    ...
    Public Function MostrarServicio(idServicio) As Servicios
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select * from servicio where IDServicio=@idservicio and Estatus=1"
    ...
    Public Sub Editarservicio(idservicio, idtiposervicio, nombre, costo, IDUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE Servicio SET TipoServicio= @idtiposervicio, Nombre= @nombre, Costo=@costo, IdUsuarioModifica=@idusuariomodifica, FechaModifica=GETDATE() WHERE IDServicio=@idservicio"
                    command.Parameters.AddWithValue("@idservicio", idservicio)
    ...
    Public Function Mascotasparacitasdeclientes(idcliente) As List(Of Mascotas)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT IDMascota, IDCliente, Nombre AS NombreMascota, Color FROM Mascota M WHERE IDCliente = @idcliente and fallecido=0"
                    command.Parameters.AddWithValue("@idcliente", idcliente)
                    command.CommandType = CommandType.Text
                    Dim reader = command.ExecuteReader()
    ...
    Public Sub InsertarCita(idMascota, idhorario, idecita, fechacita, descripcion, IdUsuarioCrea)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO Cita (IDMascota, IDHorario, IDECita, Fechacita, Descripcion, IdUsuarioCrea) VALUES (@IDMascota, @IDHorario, @IDECita, @Fechacita, @Descripcion, @idusuariocrea)"
                    command.Parameters.AddWithValue("@IDMascota", idMascota)
    ...
    Public Function obtenerHORARIOCITAS() As List(Of Citas)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT IDHorario, CONCAT(CONVERT(varchar(5), HoraInicio, 108), ' - ', CONVERT(varchar(5), HoraFin, 108)) AS HorarioCompleto FROM Horario"
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
    ...
    Public Function cargarcitasprimerainstancia(fecha) As DataTable
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado from Cita c join Mascota m on m.IDMascota=c.IDMascota join Horario h on h.IDHorario=c.IDHorario join Estadocita e on e.IDECita=c.IDECita where c.Fechacita=@fechacita order by Nombre asc"
    ...
    Public Function cargarcitaspendientes(fecha) As DataTable
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado from Cita c join Mascota m on m.IDMascota=c.IDMascota join Horario h on h.IDHorario=c.IDHorario join Estadocita e on e.IDECita=c.IDECita where c.Fechacita=@fechacita and e.Estado= 'Pendiente' order by HorarioCompleto asc"
                    command.Parameters.AddWithValue("@fechacita", fecha)
                    command.CommandType = CommandType.Text
    ...
    Public Function cargarcitasconfirmadas(fecha) As DataTable
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado from Cita c join Mascota m on m.IDMascota=c.IDMascota join Horario h on h.IDHorario=c.IDHorario join Estadocita e on e.IDECita=c.IDECita where c.Fechacita=@fechacita and e.Estado= 'Confirmada' order by HorarioCompleto asc"
                    command.Parameters.AddWithValue("@fechacita", fecha)
                    command.CommandType = CommandType.Text
    ...
    Public Function cargarcitascompletadas(fecha) As DataTable
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado from Cita c join Mascota m on m.IDMascota=c.IDMascota join Horario h on h.IDHorario=c.IDHorario join Estadocita e on e.IDECita=c.IDECita where c.Fechacita=@fechacita and e.Estado= 'Completada' order by HorarioCompleto asc"
                    command.Parameters.AddWithValue("@fechacita", fecha)
                    command.CommandType = CommandType.Text
    ...
    Public Function cargarcitascanceladas(fecha) As DataTable
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado from Cita c join Mascota m on m.IDMascota=c.IDMascota join Horario h on h.IDHorario=c.IDHorario join Estadocita e on e.IDECita=c.IDECita where c.Fechacita=@fechacita and e.Estado= 'Cancelada' order by HorarioCompleto asc"
                    command.Parameters.AddWithValue("@fechacita", fecha)
                    command.CommandType = CommandType.Text
    ...
    Public Function mostrarcita(idcita) As Citas
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "select c.IDCita, c.Fechacita, c.IDHorario, c.IDECita, c.Descripcion as descripcioncita, cl.PrimerNombre, cl.SegundoNombre, cl.ApellidoPaterno, cl.ApellidoMaterno, cl.Email, cl.TelCasa, cl.Celular, cl.TelTrabajo, cl.TelExterno, cl.Descripcion, m.Nombre, m.Peso, r.Nombre as razanombre, t.Nombre as tiponombre, s.Nombre as sexonombre, m.Esvacunado, m.Vacuna, m.VigVacuna from Cita c join Estadocita e on e.IDECita=c.IDECita join Mascota m on m.IDMascota=c.IDMascota join SexoMascota s on s.IDSeMascota=m.IDSeMascota join RazaMascota r on r.IDRMascota=m.IDRMascota join TipoMascota t on t.IDTMascota=r.IDTMascota join Cliente cl on cl.IDCliente=m.IDCliente where c.IDCita=@idcita"
    ...
    Public Sub EditarEstadoCita(idcita, idecita, IDUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE Cita SET IDECita=@idecita, IdUsuarioModifica=@idusuariomodifica, FechaModifica=GETDATE() WHERE IDCita=@idcita"
                    command.Parameters.AddWithValue("@idcita", idcita)
                    command.Parameters.AddWithValue("@idecita", idecita)
                    command.Parameters.AddWithValue("@idusuariomodifica", IDUsuarioModifica)
                    command.CommandType = CommandType.Text
    ...
    Public Sub EditarCita(idcita, idhorario, fechacita, descripcion, IdUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE Cita SET IDHorario = @idhorario, Fechacita = @fechacita, Descripcion = @descripcion, IdUsuarioModifica=@idusuariomodifica, FechaModifica=GETDATE() WHERE IDCita = @idcita"
                    command.Parameters.AddWithValue("@idcita", idcita)
                    command.Parameters.AddWithValue("@idhorario", idhorario)
                    command.Parameters.AddWithValue("@fechacita", fechacita)
                    command.Parameters.AddWithValue("@descripcion", descripcion)
    ...
    Public Sub InsertarRazaMascota(IdTipoMascota, Nombre, IdUsuarioCrea)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO RazaMascota (IDTMascota, Nombre, IdUsuarioCrea) VALUES (@idtmascota, @mascota, @IDUsuarioCrea)"
    ...
    Public Function BuscarRazayTipoDeMascotas() As List(Of Mascotas)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT RazaMascota.IDRMascota, TipoMascota.Nombre AS NombretipoMascota, RazaMascota.Nombre AS NombreRazaMascota FROM RazaMascota JOIN TipoMascota ON TipoMascota.IDTMascota=RazaMascota.IDTMascota ORDER BY TipoMascota.Nombre, RazaMascota.Nombre"
                    command.CommandType = CommandType.Text
                    Dim reader = command.ExecuteReader()
    ...
    Public Sub EditarRaza(IdRmazcota, IdTMascota, nombre, IdUsuarioModifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE RazaMascota SET IDTMascota=@idtmascota, Nombre=@nombrerazamascota, IdUsuarioModifica=@IdUsuarioModifica, FechaModifica=GETDATE() WHERE IDRMascota=@idrmascota"
    ...
    Public Sub InsertarUsuario(PrimerNombre, SegundoNombre, ApellidoPaterno, ApellidoMaterno, Email, Usuario, contraseña, idrol)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO Trabajador (PrimerNombre, SegundoNombre, ApellidoPaterno, ApellidoMaterno, Email, Usuario, Contraseña, IDRol, FCreacion) VALUES (@PrimerNombre, @SegundoNombre, @ApellidoPaterno, @ApellidoMaterno, @Email, @Usuario,EncryptByPassPhrase('petpalace', CONVERT(varbinary, @Contraseña)), @IDRol, GETDATE())"
                    command.Parameters.AddWithValue("@PrimerNombre", PrimerNombre)
                    command.Parameters.AddWithValue("@SegundoNombre", SegundoNombre)
                    command.Parameters.AddWithValue("@ApellidoPaterno", ApellidoPaterno)
                    command.Parameters.AddWithValue("@ApellidoMaterno", ApellidoMaterno)
    ...
    Public Function ObtenerServiciosdeTIPOS(TipoServicio) As List(Of Servicios)
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT * FROM Servicio WHERE TipoServicio=@IdTServicios AND Estatus=1 ORDER BY Nombre ASC"
    ...
    Public Sub InsertarDetallesCitasServicios(IdCita, IdServicio, IdUsuarioCrea)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "INSERT INTO DetallesCitaServicio (IDCita,IDServicio,IdUsuarioCrea) VALUES (@Idcita,@IdServicio,@IdUsuarioCrea)"
                    command.Parameters.AddWithValue("@Idcita", IdCita)
                    command.Parameters.AddWithValue("@IdServicio", IdServicio)
                    command.Parameters.AddWithValue("@IdUsuarioCrea", IdUsuarioCrea)
                    command.CommandType = CommandType.Text
    ...
    Public Function ObtenerServiciosDetallesCitas(idcita As Integer) As DataTable
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT DetallesCitaServicio.IDDetallesCS AS Codigo, tipoServicio.Nombre AS 'Tipo de Servicio', Servicio.Nombre AS 'Nombre de Servicio', ROUND(Servicio.Costo, 2) AS 'Precio' FROM DetallesCitaServicio JOIN Servicio ON Servicio.IDServicio = DetallesCitaServicio.IDServicio JOIN tipoServicio ON tipoServicio.TipoServicio = Servicio.TipoServicio JOIN Cita ON cita.IDCita = DetallesCitaServicio.IDCita WHERE Cita.IDCita = @idcita and DetallesCitaServicio.Estatus=1"
                    command.Parameters.AddWithValue("@idcita", idcita)
                    command.CommandType = CommandType.Text
    ...
    Public Function CargarHistorialCitas(idmascota) As DataTable
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT c.IDCita, m.Nombre, c.Fechacita, CONCAT(CONVERT(varchar(5), h.HoraInicio, 108), ' - ', CONVERT(varchar(5), h.HoraFin, 108)) AS HorarioCompleto, e.Estado FROM Cita c JOIN Mascota m on m.IDMascota=c.IDMascota JOIN Horario h on h.IDHorario=c.IDHorario JOIN Estadocita e on e.IDECita=c.IDECita WHERE m.IDMascota=@idmascota ORDER BY c.Fechacita ASC"
                    command.Parameters.AddWithValue("@idmascota", idmascota)
                    command.CommandType = CommandType.Text
    ...
    Public Function MostrarCostototal(idcita As Integer) As Servicios
        ...
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "SELECT ROUND(SUM(servicio.costo), 2) AS TOTAL FROM DetallesCitaServicio JOIN Servicio ON Servicio.IDServicio = DetallesCitaServicio.IDServicio JOIN tipoServicio ON tipoServicio.TipoServicio = Servicio.TipoServicio JOIN Cita ON cita.IDCita = DetallesCitaServicio.IDCita WHERE Cita.IDCita = @idcita and DetallesCitaServicio.Estatus=1"
                    command.Parameters.AddWithValue("@idcita", idcita)
                    command.CommandType = CommandType.Text
    ...
    Public Sub EliminarServicio(idDetalleServicio, idusuariomodifica)
        Try
            Using connection = GetConnection()
                ...
                Using command = New SqlCommand()
                    command.Connection = connection
                    command.CommandText = "UPDATE DetallesCitaServicio SET Estatus=0, IdUsuarioModifica=@idusuariomodifica, FechaModifica=GETDATE() WHERE IDDetallesCS = @iddetallescita"
                    ' Asegúrate de configurar los parámetros correspondientes
